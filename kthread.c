...
static void *ktp_worker(void *data)
{
	...
	while (w->step < p->n_steps) {
		...
		// working on w->step
		w->data = p->func(p->shared, w->step, w->step? w->data : 0, w->index); // for the first step, input is NULL
...
void kt_pipeline(int n_threads, void *(*func)(void*, int, void*, int), void *shared_data, int n_steps)
{
	ktp_t aux;
	pthread_t *tid;
	int i;
